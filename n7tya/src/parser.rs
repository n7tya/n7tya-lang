...
impl Parser {
        ...
        let body = self.parse_indented_block(|parser| {
             ...
             if let Some(Token::Identifier(id)) = parser.peek_token().cloned() {
                 ...
                 if parser.match_token(Token::Colon) {
                     ...
                     return Ok(Some(ClassBodyItem::Field(FieldDef { name: id, type_annotation })));
                 } else {
    ...
    fn parse_server_def(&mut self) -> Result<ServerDef> {
        ...
        let body = self.parse_indented_block(|parser| {
             ...
             if let Some(Token::StringLiteral(path)) = path_token {
                 ...
                 return Ok(Some(ServerBodyItem::Route(RouteDef {
                     path,
                     method, 
                     body,
                 })));
             } else {
                 return Err(miette::miette!("Expect string literal (path) after route method, got {:?}", parser.peek_token()));
             }
        })?;
    ...
    fn parse_function_def(&mut self) -> Result<FunctionDef> {
        ...
        while !self.check(Token::Newline) && !self.check(Token::Arrow) && !self.check(Token::Colon) && !self.is_at_end() {
    ...
    fn parse_statement(&mut self) -> Result<Option<Statement>> {
        ...
        if let Ok(expr) = self.parse_expression() {
            if self.match_token(Token::Assign) {
                ...
                self.match_token(Token::Newline);
                return Ok(Some(Statement::Assignment(AssignmentStmt { target: expr, value })));
            }
    ...
    fn parse_let(&mut self) -> Result<LetDecl> {
        ...
        let value = self.parse_expression()?;
        self.match_token(Token::Newline);
        Ok(LetDecl { name, value, type_annotation })
    }
    ...
    fn parse_const(&mut self) -> Result<ConstDecl> {
        ...
        let value = self.parse_expression()?;
        self.match_token(Token::Newline);
        Ok(ConstDecl { name, value, type_annotation })
    }
    ...
    fn parse_import(&mut self) -> Result<ImportStmt> {
        ...
        self.match_token(Token::Newline);
        Ok(ImportStmt { module, names: vec![], alias })
    }
    ...
    fn parse_from_import(&mut self) -> Result<ImportStmt> {
        ...
        self.match_token(Token::Newline);
        Ok(ImportStmt { module, names, alias: None })
    }
    ...
    fn parse_for(&mut self) -> Result<ForStmt> {
        ...
        let body = self.parse_block()?;
        Ok(ForStmt { target, iterator, body })
    }
    ...
    fn parse_comparison(&mut self) -> Result<Expression> {
        ...
        while self.match_token(Token::Lt) || self.match_token(Token::Gt) ||
              self.match_token(Token::LtEq) || self.match_token(Token::GtEq) {
             let op = match self.previous().token {
    ...
    fn is_arg_start(&self) -> bool {
        if let Some(token) = self.peek_token() {
             match token {
                 Token::Identifier(_) | Token::IntLiteral(_) | Token::StringLiteral(_) | 
                 Token::FloatLiteral(_) |
                 Token::LParen | Token::LBrace | Token::LBracket |Token::SelfKw => true,
                 _ => false,
             }
        } else {
            false
    ...
    fn parse_postfix(&mut self) -> Result<Expression> {
        ...
        loop {
            if self.match_token(Token::Dot) {
                ...
                expr = Expression::MemberAccess(Box::new(MemberExpr { object: expr, member }));
    ...
    fn parse_atom(&mut self) -> Result<Expression> {
        ...
        Err(miette::miette!("Expect expression, got {:?}", self.peek_token()))
    }
    ...
    fn parse_jsx_element(&mut self) -> Result<JsxElement> {
        ...
        while !self.check(Token::Gt) && !self.check(Token::SelfClose) && !self.is_at_end() {
             if let Ok(name) = self.consume_identifier("") {
                 ...
                 self.advance();
                 while !self.check(Token::RBrace) && !self.is_at_end() { self.advance(); }
                 self.match_token(Token::RBrace);
             } else {
                 // 未知のトークンが進まなくなるのを防ぐ
                 self.advance();
        ...
        if self.match_token(Token::SelfClose) {
            return Ok(JsxElement { tag, attributes, children: Vec::new() });
        }
        ...
        if tag != close_tag {
            return Err(miette::miette!("Tag mismatch: <{}> ... </{}>", tag, close_tag));
        }
        
        self.consume(Token::Gt, "Expect '>' after close tag")?;
        
        Ok(JsxElement { tag, attributes, children })
    }
    ...
    fn peek_token(&self) -> Option<&Token> {
        if self.is_at_end() { None } else { Some(&self.tokens[self.current].token) }
    }
