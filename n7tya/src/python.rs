...
pub fn get_python_function(module_name: &str, func_name: &str) -> Result<PyObject, String> {
    Python::with_gil(|py| {
        let module = py.import(module_name)
            .map_err(|e| format!("Failed to import '{}': {}", module_name, e))?;
        module.getattr(func_name)
            .map(|f| f.into_py(py))
            .map_err(|e| format!("Failed to get '{}' from '{}': {}", func_name, module_name, e))
    })
}
...
pub fn call_python_function(func: &PyObject, args: Vec<Value>) -> Result<Value, String> {
    Python::with_gil(|py| {
        ...
        let result = func.call1(py, PyTuple::new(py, &py_args).unwrap())
...
pub fn py_to_value(py: Python, obj: &PyObject) -> Result<Value, String> {
    ...
    if let Ok(list) = obj_ref.downcast::<PyList>() {
        let items: Result<Vec<Value>, String> = list.iter()
...
pub fn install_python_package(package: &str) -> Result<(), String> {
    Python::with_gil(|py| {
        ...
        subprocess.call_method1("run", (args,))
...
impl PythonModule {
    ...
    pub fn call(&self, func_name: &str, args: Vec<Value>) -> Result<Value, String> {
        Python::with_gil(|py| {
            let func = self.module.bind(py).getattr(func_name)
            ...
            let result = func.call1(PyTuple::new(py, &py_args).unwrap())
    ...
    pub fn get_attr(&self, name: &str) -> Result<Value, String> {
        Python::with_gil(|py| {
            let attr = self.module.bind(py).getattr(name)
