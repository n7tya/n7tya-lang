...
pub enum N7tyaError {
    ...
    #[diagnostic(code(n7tya::undefined), help("Did you forget to declare '{name}' with 'let'?"))]
    UndefinedVariable {
        name: String,
        #[label("not found")]
        span: Option<SourceSpan>,
                ...
                match error {
                    N7tyaError::Syntax { message, span } => {
                        ...
                        if line < lines.len() {
                            ...
                            eprintln!("  {} | {}^", " ".repeat((line + 1).to_string().len()), " ".repeat(col));
                        }
                    }
                    N7tyaError::Type { message, span } => {
                        if let Some(span) = span {
                            let (line, col) = offset_to_line_col(source, span.offset());
                            eprintln!("{}:{}:{}: type error: {}", filename, line + 1, col + 1, message);
                        } else {
                    ...
                    N7tyaError::UndefinedVariable { name, span } => {
                        if let Some(span) = span {
                            let (line, col) = offset_to_line_col(source, span.offset());
                            eprintln!("{}:{}:{}: error: undefined variable '{}'", filename, line + 1, col + 1, name);
                        } else {
